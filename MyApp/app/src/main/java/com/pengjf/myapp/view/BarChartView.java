...
public class BarChartView extends View {
    ...
    private void init(Context context) {
        ...
        mContext = context;
        leftMargin = DisplayUtil.dp2px(context, 16);
        topMargin = DisplayUtil.dp2px(context, 30);

        bgPaint = new Paint();
        bgPaint.setColor(Color.WHITE);

        axisPaint = new Paint();
        axisPaint.setStrokeWidth(DisplayUtil.dp2px(context, 1));

        textPaint = new Paint();
        textPaint.setAntiAlias(true);
        textPaint.setTextSize(DisplayUtil.dp2px(getContext(), 10));
...
//            }
    ...
    private void getItemsWidth() {
        int barMinWidth = DisplayUtil.dp2px(getContext(), 40);
        int barMinSpace = DisplayUtil.dp2px(getContext(), 10);
    ...
    private void drawXAxisText(Canvas canvas) {
        ...
        for (int i = 0; i < mData.size(); i++) {
            ...
            if ((xStartIndex + distance) >= xStartIndex && (xStartIndex + distance) < (mTotalWidth - leftMargin * 2)) {
                canvas.drawText(text, mBarLeftXPoints.get(i) - (textPaint.measureText(text) - barWidth) / 2, paintBottom + DisplayUtil.dp2px(getContext(), 10), textPaint);
            }
        }
    }
