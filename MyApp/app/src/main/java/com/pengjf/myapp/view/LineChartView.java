...
/**
 * Created by jiangfeng  on 2017/3/29 0029 10:55
...
public class LineChartView extends View {
     ...
     * 当前移动的距离
     */
    private float movingThisDisTance= 0.0f;
    ...
    private void init(Context context) {
        mContext = context;
        leftMargin = DisplayUtil.dp2px(mContext, 16);
        topMargin = DisplayUtil.dp2px(mContext, 30);
        bgPaint = new Paint();
        bgPaint.setColor(Color.WHITE);
        bgPaint.setAntiAlias(true);

        axisPaint = new Paint();
        axisPaint.setStrokeWidth(DisplayUtil.dp2px(mContext, 1));

        textPaint = new Paint();
        textPaint.setAntiAlias(true);
        textPaint.setTextSize(DisplayUtil.dp2px(mContext, 10));

        linePaint = new Paint();
        linePaint.setAntiAlias(true);
        linePaint.setStrokeWidth(DisplayUtil.dp2px(mContext, 1));
    ...
    private void drawXAxisText(Canvas canvas) {
        ...
        for(int i = 0;i<mData.size();i++){
            ...
            if((xStartIndex+distance)>=xStartIndex&&(xStartIndex+distance)<(mTotalWidth-leftMargin*2)){
                canvas.drawText(text, xStartIndex+distance-textPaint.measureText(text)/2, paintBottom + DisplayUtil.dp2px(getContext(), 10), textPaint);
            }
        }
    }
...
//                canvas.drawLine(xStartIndex, startY, mTotalWidth - leftMargin*2, startY, axisPaint);
    ...
    private void getItemsWidth() {
        space = DisplayUtil.dp2px(getContext(), 30);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()){
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                movingThisDisTance = lastPointX - moveX;
                leftMoving = leftMoving+ movingThisDisTance ;
                lastPointX = moveX ;
                invalidate();
                break;
            case MotionEvent.ACTION_UP:
                new Thread(new SmoothScrollThread(movingThisDisTance)).start();
                break;
            default:
